...
pub struct Santa;
impl Santa {
    pub fn give_gift<R: Giftable, G: Gift>(&self, recipient: &mut R, gift: &G) -> Result<(), Box<dyn Error>> {
        // 5. Update the function signature to accept any type of recipient and gift
        match gift.is_wrapped() {
            false => Err("gift not wrapped".into()),
            true => {recipient.receive_gift();
            Ok(())},
        }
    }
}
